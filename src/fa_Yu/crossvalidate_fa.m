 ...
 rng shuffle;
  ...
  for i = 1:length(zDimList)
    ...
    for cvf = 0:numFolds      
      ...
      % Check if training data covariance is full rank (changed by s.wu July 16 2020)
      if rcond(cov(Xtrain')) <1e-8
        error('Training data covariance matrix singular.\n');
      return
        %keyboard
      end
      ...
      if zDim == 0
        ...
        estParams = fastfa(Xtrain, zDim, 'minVarFrac', -Inf,'tol',tol);
      end
